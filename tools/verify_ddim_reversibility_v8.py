...
def make_selected_positions(shape, exclude_channels, device):
    ...
    for c in range(channels):
        if c in excluded:
            continue
        start = c * time_steps * freq_bins
        end = (c + 1) * time_steps * freq_bins
...
def run_bit_embedding_experiment(
    ...
    selected_positions = make_selected_positions(shape, exclude_channels, device)
    if selected_positions.numel() == 0:
        raise ValueError("No latent positions left after channel exclusion.")
