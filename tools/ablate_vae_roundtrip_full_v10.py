...
def main():
    ...
    parser.add_argument(
        ...
        default=None,
        help="For bits_per_z=1, choose the threshold used for final decoded bits. Defaults to source setting or oracle.",
    )
    parser.add_argument(
        "--decode_threshold",
        type=float,
    ...
    calibrate_threshold = (
        ...
        else source_settings.get("calibrate_threshold", "oracle")
    )
    decode_threshold = (
        args.decode_threshold
        if args.decode_threshold is not None
        else source_settings.get("decode_threshold")
    )
    ...
    with torch.no_grad():
        ...
        all_default_bits, all_default_symbols = decode_gaussian_bins(vae_recovered_zs, bits_per_z)
        ...
        default_decoded_bits = all_default_bits[used_positions]
        default_decoded_symbols = all_default_symbols[used_positions]
        decoded_bits = default_decoded_bits
        decoded_symbols = default_decoded_symbols
        ...
        message_bits = None
        payload_bits = bit_data.get("payload_bits")
        ecc_settings = bit_data.get("ecc_settings")
        if payload_bits is not None:
            payload_bits = payload_bits.to(model.device)
        if (
        ...
        selected_threshold_mode = "none"
        if bits_per_z == 1:
            if decode_threshold is not None:
                selected_threshold = torch.tensor(float(decode_threshold), device=model.device)
                selected_threshold_mode = "manual"
            elif calibrate_threshold == "median":
                selected_threshold = torch.tensor(
                    calibration_metrics["median_threshold"], device=model.device
                )
                selected_threshold_mode = "median"
            elif calibrate_threshold == "oracle":
                selected_threshold = torch.tensor(
                    calibration_metrics["oracle_best_threshold"], device=model.device
                )
                selected_threshold_mode = "oracle"
        calibration_metrics["selected_threshold_mode"] = selected_threshold_mode
        calibration_metrics["selected_threshold"] = (
            None if selected_threshold is None else float(selected_threshold.item())
        )
        if selected_threshold is not None:
            decoded_bits, decoded_symbols = decode_one_bit_with_threshold(
                vae_recovered_zs.reshape(-1)[used_positions], selected_threshold
            )
        if grouped_positions is not None and message_bits is not None:
            local_grouped_positions = torch.arange(
                decoded_bits.shape[0], device=decoded_bits.device
            ).reshape(grouped_positions.shape)
            decoded_message_bits, vote_metrics = repetition_vote_metrics(
                message_bits,
                decoded_bits,
                local_grouped_positions,
            )
        if ecc_settings is not None and ecc_settings.get("ecc") == "bch":
            ...
            decoded_payload_bits = decoded_payload_bits.to(model.device)
            corrected_code_bits = corrected_code_bits.to(model.device)

    position_metadata = metadata_to_plain_dict(bit_data.get("position_metadata"))
    exclude_channels = tensor_to_int_list(bit_data.get("exclude_channels"))
    selected_positions = bit_data.get("selected_positions")
    selected_count = (
        int(selected_positions.numel()) if torch.is_tensor(selected_positions) else None
    )
    used_count = int(used_positions.numel())
    ...
    report = {
        "settings": {
            ...
            "ecc_settings": ecc_settings,
            "exclude_channels": exclude_channels,
            "selected_positions": selected_count,
            "used_positions": used_count,
            **position_metadata,
        },
    ...
    torch.save(
            {
                ...
                "default_decoded_bits": default_decoded_bits.cpu(),
                "default_decoded_symbols": default_decoded_symbols.cpu(),
                "threshold_calibration": calibration_metrics,
                "used_positions": used_positions.detach().cpu(),
                ...
                "decoded_payload_bits": None
                if decoded_payload_bits is None
                else decoded_payload_bits.cpu(),
                "corrected_code_bits": None
                if corrected_code_bits is None
                else corrected_code_bits.cpu(),
                "ecc_metrics": ecc_metrics,
            },
            save_pt_path,
        )
    print(f"Saved report to {report_json_path}")
    print(f"Saved tensors to {save_pt_path}")
